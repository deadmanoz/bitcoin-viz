...
class SigningAndInputConstruction(Scene):
    ...
    def construct(self):
        ...
        # Final caption
        ready_text = Text(
            "Ready to construct the complete transaction",
            font_size=26,
            color=SYNTH_CYAN,
            weight=BOLD
        )
    ...
    def create_private_key_visual(self):
        ...
        # Key icon (simple key shape) - larger
        key_circle = Circle(radius=0.4, color=SYNTH_PURPLE, stroke_width=4)
        key_circle.set_fill(color=SYNTH_PURPLE, opacity=0.4)
        ...
        key_shaft.set_fill(color=SYNTH_PURPLE, opacity=0.4)
        key_shaft.next_to(key_circle, RIGHT, buff=0)

        key_icon = VGroup(key_circle, key_shaft)

        # Label - larger and brighter
        key_label = Text("Private Key", font_size=18, color=SYNTH_PURPLE, weight=BOLD)
        key_label.next_to(key_icon, DOWN, buff=0.3)

        # Hex representation - larger and more visible
        hex_text = Text("5Kb8...3Qm", font_size=14, color=SYNTH_PURPLE)
    ...
    def create_public_key_visual(self):
        ...
        # Public key icon (circle with broadcast symbol) - larger
    ...
    def sign_utxo_and_create_input(self, utxo, txid, vout, keys_group, explanation, utxo_num):
        ...
        self.play(Create(lightning), run_time=1)
        self.wait(1)
